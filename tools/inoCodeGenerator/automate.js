...
function automatePages(language) {
  ...
  pagesHeader = pagesHeader.replace('<<<MENUHTML>>>', minify.minifyFileByPath('../../router-esp32/pages/' + language + '/menu.html'));
  pagesHeader = pagesHeader.replace('<<<REFEREEHTML>>>', minify.minifyFileByPath('../../router-esp32/pages/' + language + '/referee.html'));
...
function generateInoCredentials() {
  ...
  fs.writeFile('../../router-esp32/EasyLifterRefereeESP32/EasyLifterRefereeESP32.ino', inoContents, function (err) {
    if (err) {
      return console.log(err);
    }
    console.log('Done');
...
console.log('Generating EasyLifterRefereeESP32.ino with ssid and password from credentials.json:');
